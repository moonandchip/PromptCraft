...
from app.round.models import RoundAttemptResponse, RoundInfo, RoundStartResponse, RoundSubmitRequest, RoundSubmitResponse


class TestRoundSubmitRequest(unittest.TestCase):
    def test_valid_submit_request(self):
        req = RoundSubmitRequest(round_id="ancient-temple", user_prompt="a beautiful ancient temple")
    ...
    def test_empty_round_id_raises_validation_error(self):
        with self.assertRaises(ValidationError):
            RoundSubmitRequest(round_id="", user_prompt="a valid prompt")

    def test_empty_user_prompt_raises_validation_error(self):
        with self.assertRaises(ValidationError):
            RoundSubmitRequest(round_id="some-round", user_prompt="")

    def test_round_id_exceeds_max_length_raises_validation_error(self):
        with self.assertRaises(ValidationError):
            RoundSubmitRequest(round_id="x" * 101, user_prompt="a valid prompt")
    ...
    def test_user_prompt_at_max_length_is_valid(self):
        req = RoundSubmitRequest(round_id="some-round", user_prompt="x" * 2000)
        self.assertEqual(len(req.user_prompt), 2000)


class TestRoundSubmitResponse(unittest.TestCase):
    def test_default_similarity_score_is_zero(self):
        resp = RoundSubmitResponse(generated_image_url="https://example.com/image.jpg")
        self.assertEqual(resp.similarity_score, 0.0)

    def test_explicit_similarity_score(self):
        resp = RoundSubmitResponse(
    ...
    def test_generated_image_url_is_stored(self):
        ...
        resp = RoundSubmitResponse(generated_image_url=url)
        self.assertEqual(resp.generated_image_url, url)

    def test_similarity_score_zero_boundary(self):
        resp = RoundSubmitResponse(generated_image_url="https://example.com/img.jpg", similarity_score=0.0)
        self.assertEqual(resp.similarity_score, 0.0)

    def test_similarity_score_max_boundary(self):
        resp = RoundSubmitResponse(generated_image_url="https://example.com/img.jpg", similarity_score=100.0)
...
class TestRoundStartResponse(unittest.TestCase):
    ...
    def test_start_round_response_missing_field_raises_validation_error(self):
        with self.assertRaises(ValidationError):
            RoundStartResponse(round_id="ancient-temple")


class TestRoundAttemptResponse(unittest.TestCase):
    def test_attempt_info_fields(self):
        attempt = RoundAttemptResponse(
            attempt_number=2,
